...
class Packager {
    ...
    installAppDependencies(platform, arch) {
        ...
        return (0, (_bluebirdLstC || _load_bluebirdLstC()).coroutine)(function* () {
            if (_this3.devMetadata.build.nodeGypRebuild === true) {
            ...
            if (_this3.isTwoPackageJsonProjectLayoutUsed) {
                if (_this3.devMetadata.build.npmRebuild === false) {
                    (0, (_log || _load_log()).log)("Skip app dependencies rebuild because npmRebuild is set to false");
                } else {
                    const forceBuildFromSource = _this3.devMetadata.build.npmSkipBuildFromSource !== true;
                    if (forceBuildFromSource && platform.nodeName !== process.platform) {
                        ...
                        if (yield (0, (_util || _load_util()).exists)((_path || _load_path()).join(_this3.appDir, "node_modules"))) {
                            const args = (0, (_util || _load_util()).asArray)(_this3.devMetadata.build.npmArgs);
                            if (forceBuildFromSource) {
                                args.push("--build-from-source");
                            }
                            yield (0, (_yarn || _load_yarn()).rebuild)(_this3.appDir, _this3.electronVersion, (_metadata || _load_metadata()).Arch[arch], args);
                        } else {
                            yield (0, (_yarn || _load_yarn()).installDependencies)(_this3.appDir, _this3.electronVersion, (_metadata || _load_metadata()).Arch[arch], forceBuildFromSource, _this3.devMetadata.build.npmArgs);
                        }
                    }
                }
            } else {
                (0, (_log || _load_log()).log)("Skip app dependencies rebuild because dev and app dependencies are not separated");
            }
        })();
    }
}
