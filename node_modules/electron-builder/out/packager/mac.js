...
let createApp = exports.createApp = (() => {
    var _ref = (0, (_bluebirdLstC || _load_bluebirdLstC()).coroutine)(function* (packager, appOutDir, initializeApp) {
        const appInfo = packager.appInfo;
        const appFilename = appInfo.productFilename;
        const contentsPath = (_path || _load_path()).join(appOutDir, "Electron.app", "Contents");
        const frameworksPath = (_path || _load_path()).join(contentsPath, "Frameworks");
        const appPlistFilename = (_path || _load_path()).join(contentsPath, "Info.plist");
        const helperPlistFilename = (_path || _load_path()).join(frameworksPath, "Electron Helper.app", "Contents", "Info.plist");
        const helperEHPlistFilename = (_path || _load_path()).join(frameworksPath, "Electron Helper EH.app", "Contents", "Info.plist");
        const helperNPPlistFilename = (_path || _load_path()).join(frameworksPath, "Electron Helper NP.app", "Contents", "Info.plist");
        const buildMetadata = packager.devMetadata.build;
        const result = yield (_bluebirdLstC2 || _load_bluebirdLstC2()).default.all([initializeApp(), (_bluebirdLstC2 || _load_bluebirdLstC2()).default.map([appPlistFilename, helperPlistFilename, helperEHPlistFilename, helperNPPlistFilename, buildMetadata["extend-info"]], function (it) {
        ...
        if (oldCategory != null) {
            (0, (_log || _load_log()).warn)("app-category-type is deprecated, please set as build.mac.category");
        }
        let category = packager.platformSpecificBuildOptions.category || buildMetadata.category || oldCategory;
        (0, (_util || _load_util()).use)(category || oldCategory, function (it) {
            return appPlist.LSApplicationCategoryType = it;
        });
        (0, (_util || _load_util()).use)(appInfo.copyright, function (it) {
            return appPlist.NSHumanReadableCopyright = it;
        });
        ...
        if (icon != null) {
            promises.push((0, (_fsExtraP || _load_fsExtraP()).unlink)((_path || _load_path()).join(contentsPath, "Resources", oldIcon)));
            promises.push((0, (_fsExtraP || _load_fsExtraP()).copy)(icon, (_path || _load_path()).join(contentsPath, "Resources", appPlist.CFBundleIconFile)));
        }
        yield (_bluebirdLstC2 || _load_bluebirdLstC2()).default.all(promises);
        yield moveHelpers(frameworksPath, appFilename);
        yield (0, (_fsExtraP || _load_fsExtraP()).rename)((_path || _load_path()).dirname(contentsPath), (_path || _load_path()).join(appOutDir, `${ appFilename }.app`));
    });
...
function doRename(basePath, oldName, newName) {
    return (0, (_fsExtraP || _load_fsExtraP()).rename)((_path || _load_path()).join(basePath, oldName), (_path || _load_path()).join(basePath, newName));
}
function moveHelpers(frameworksPath, appName) {
    return (_bluebirdLstC2 || _load_bluebirdLstC2()).default.map([" Helper", " Helper EH", " Helper NP"], suffix => {
        const executableBasePath = (_path || _load_path()).join(frameworksPath, `Electron${ suffix }.app`, "Contents", "MacOS");
